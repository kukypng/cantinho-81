...
const NavigationLinks = memo(({ isMobile }: { isMobile: boolean }) => (
  !isMobile && (
...
const SocialMediaLinks = memo(({ socialMedia }: { socialMedia: any }) => (
                ...
                <Sheet>
                  ...
                  <SheetContent side="right">
                    <nav className="flex flex-col gap-4 pt-10">
                      {navigationItems.map(item => (
                        <Link key={item.name} to={item.href} className="text-lg font-medium">
                          {item.name}
                        </Link>
                      ))}
                    </nav>
