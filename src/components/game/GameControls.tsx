...
const GameControls: React.FC<GameControlsProps> = ({
  ...
  return <>
      {/* Game info panel */}
      <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-gray-200 py-[10px]">
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm space-y-1">
            <div><span className="font-bold text-store-pink">Movimentos:</span> {moves}</div>
            <div className="py-[6px]"><span className="font-bold text-store-pink">Tempo:</span> {formatTime(time)}</div>
          </div>
      ...
      <div className="mb-4 flex justify-center space-x-2">
        ...
        if (!isPlaying || window.confirm('Alterar a dificuldade reiniciará o jogo. Continuar?')) {
          ...
          hard: 'Difícil'
            ...
            {stats.bestScores[level] && <div className="absolute -top-1 -right-1 w-2 h-2 bg-yellow-400 rounded-full"></div>}
          </Button>)}
      </div>
