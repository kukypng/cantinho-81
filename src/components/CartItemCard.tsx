...
const CartItemCard: React.FC<CartItemCardProps> = ({ item, allowEdit = true }) => {
  ...
  return (
    <div className="group bg-white/95 backdrop-blur-sm border border-gray-100 rounded-lg p-2 transition-all hover:shadow-md hover:border-gray-200/80 hover:bg-white">
      <div className="flex items-center">
        ...
        {allowEdit && (
          <div className={`flex ${isMobile ? 'flex-col gap-1 ml-1' : 'items-center gap-2'}`}>
            <div className="flex items-center bg-gray-100 rounded-full px-1 shadow-sm group-hover:bg-gray-200 transition-colors">
              <Button
                variant="ghost"
                size="icon"
                className={`${isMobile ? 'h-5 w-5' : 'h-6 w-6'} rounded-full p-0 text-gray-600 hover:bg-gray-200 hover:text-gray-800`}
            ...
            <Button
              variant="ghost"
              size="icon"
              className={`${isMobile ? 'h-5 w-5' : 'h-6 w-6'} rounded-full text-red-500 hover:bg-red-50 hover:text-red-600`}
